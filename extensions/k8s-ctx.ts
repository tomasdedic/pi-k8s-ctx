...
interface K8sCtxConfig {
	configDir: string;
	tmpFile: string;
}

async function loadConfig(): Promise<K8sCtxConfig> {
	...
	try {
		...
		const config = JSON.parse(raw) as Partial<K8sCtxConfig>;
		return {
...
function runScript(args: string[]): { ok: boolean; output: string; error?: string } {
...
async function getCurrentConfig(): Promise<string | null> {
	const config = await loadConfig();
	try {
...
export default function k8sCtxExtension(pi: ExtensionAPI) {
